...
import { TextNode, EditorNode, isContainerNode, ContainerNode, getNodeTextContent } from '../lib';
...
export function Block({
  ...
  if (isContainerNode(node)) {
    ...
    const containerClasses = isFlexContainer
      ? `flex flex-row gap-${gap || '4'} items-start ${flexWrap === 'wrap' ? 'flex-wrap items-center' : ''}` // Flex layout for images with optional wrapping
      : isListContainer
    ...
    return (
      ...
      >
        {containerNode.children.map((childNode) => {
          ...
          const isChildImage = childNode && 'type' in childNode && childNode.type === 'img';
          ...
          const blockContent = (
            <Block
              ...
              onClick={onClick}
              onDelete={isChildImage && onDelete ? () => onDelete(childNode.id) : undefined}
      ...
      return textNode.lines!.map((line, index) => {
        ...
        if (line.children && line.children.length > 0) {
          lineContent = line.children.map(child => {
            ...
            // If it's a link
            if (child.href) {
              const linkClasses = ['hover:underline cursor-pointer', classes].filter(Boolean).join(' ');
  ...
  }, [hasChildren, hasLines, textNode.children, textNode.lines, textNode.content, textNode.type, escapeHTML]);
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === 'Enter' && e.shiftKey && isListItem) {
      console.log('🔷 [Block.tsx] Shift+Enter in list item - adding line break', {
        nodeId: textNode.id,
    ...
    if (e.key === 'Enter' && e.shiftKey && !isListItem && onCreateNested) {
      console.log('🔷 [Block.tsx] Shift+Enter in non-list item - creating nested block');
      e.preventDefault();
    ...
    if (e.key === 'Enter' && !e.shiftKey && isListItem) {
      console.log('🔷 [Block.tsx] Regular Enter in list item - creating new list item', {
        nodeId: textNode.id,
        nodeType: textNode.type,
      });
  ...
  const ElementType = textNode.type === 'li' ? 'li' : 
                      textNode.type === 'h1' ? 'h1' :
                      textNode.type === 'h2' ? 'h2' :
                      textNode.type === 'h3' ? 'h3' :
                      textNode.type === 'h4' ? 'h4' :
                      ...
                      textNode.type === 'h6' ? 'h6' :
                      textNode.type === 'p' ? 'p' :
                      textNode.type === 'blockquote' ? 'blockquote' :
                      textNode.type === 'code' ? 'pre' :
                      'div';
